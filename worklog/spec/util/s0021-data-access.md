...
- Data access does not load, parse, or validate files — it resolves paths.
...
- Data access does not perform I/O — it is a pure path resolver.
...
- File reading methods (`read`, `readBuffer`) may be added to `DataAccess` later.
- Locale-aware resolution (e.g., `data/{locale}/`) may be added later.

## Dangers

- Adding file I/O to data access would mix path resolution with resource loading, making the API harder to reason about.
