...
- Create `packages/peranto/src/error.ts` with a base class and named error classes for every failure condition currently thrown as bare `Error` in the core package.
- Implement `PerantoError` as the abstract base class extending `Error`.
- Implement: `MissingDependencyError`, `MissingHandlerError`, `DuplicateCallError`, `UnregisteredCallError`, `UndeclaredCallError`. All extend `PerantoError`.
...
- Add `CircularDependencyError` and `LifecycleStateError` extending `PerantoError` (to be used by t0006 when lifecycle is implemented). Mark these UNIMPLEMENTED in s0007.
